...
public class ReadEachDetailInEachAccount {
	...
	public void readEachDetailInEachAccount() {
		...
		// Now we could use the Assert.assertEquals to perform the assertion, but since
		// we have many assertions to perform, when the first one fails, the following
		// assertions cannot be performed
		// so instead of using Assert.assertEquals, we will use softAssert.assertEquals,
		// so that it can continue to the next assertion in case one assertion, we make
		// sure we create it's method before using it.
		// Let's fail this assertion on purpose by using 210 as the expected status code
		// instead of 200, and we will add a comment that will be printed out
		...
		softAssert.assertEquals(actualHeaderContentType, "application/json; charset=utf-8",
				"The Response Headers do not match");
		...
		// 5. Let's create an Object of a JsonPath, and then validate if the ID of the
		// first Account is available, and if it is, we will print out the id number
		...
		// 6. Using the Object of a JsonPath created above, and let's also validate if
		// the balance of the first Account is available, and if it is, we print out its
		// amount available
		...
		// Here we want to fail the assertion on purpose by not writing the correct
		// amount available in the account, and since we are using the softAssert, the
		// assertion will continue onto the next assertion
		softAssert.assertEquals(extractedFirstAccountBalance, "60000000000", "The Account Balance are not matching!!");

		// 7. Using the Object of a JsonPath created above, and let's also validate the
		// currency of the first account, and we check if it is the type of currency we
		// want (COSMIC_COINS)
		// remember we used the queryParam to search for the balance)
		...
		softAssert.assertEquals(extractedFirstAccountCurrency, "COSMIC_COINS",
				"The Account Currency are not matching!!");

		// 8. Using the Object of a JsonPath created above, let's check when the account
		// was created
		...
		System.out.println("The Account Creation Date: " + extractedFirstAccountCreationDate);
		softAssert.assertEquals(extractedFirstAccountCreationDate, "2024-09-09T07:22:30.000Z",
				"The Account creation Dates are not matching!!");

		// Using the softAssert makes it look like it is skipping each assert, now how
		// do we make sure it does perform all the assertions? we use the .assertAll()
